...
<head>
  <meta charset="UTF-8">
  <title><?= (isset($_GET["suchbegriff"]) ? '"' . htmlspecialchars($_GET["suchbegriff"], ENT_HTML5, 'UTF-8') . '" -' : ''); ?> Small Recursive Search Engine</title>
...
<body>
  ...
  <?php
  /* 
  Small Recursive Search Engine - Version at: 08/22/2021
  For an up-to-date version, see:
  https://werner-zenk.de/archiv/kleine-suchmaschine.php

  This search engine recursively searches the specified directory (and its subdirectories)
    ...
    if (
      ...
      foreach ($dateifilter as $zeiger) {
        if ($zeiger->isFile()) {
          ...
          if (!in_array(dirname($zeiger->getPathname()), $verzeichnisse)) {
            $verzeichnisse[] = dirname($zeiger->getPathname());
          }
